...
const validationSchemas = [
  ...
  Yup.object({
    personalInfo: Yup.object({
      ...
      address: Yup.string().min(5, 'Endereço deve ter pelo menos 5 caracteres'),
      linkedin: Yup.string().max(200, 'LinkedIn inválido'),
      objective: Yup.string().max(
        500,
  ...
  Yup.object({
    skills: Yup.array().test(
      'min-skills',
      'Adicione pelo menos 3 habilidades preenchidas',
      function (value) {
    ...
    languages: Yup.array().test(
      'min-languages',
      'Adicione pelo menos 1 idioma preenchido',
      function (value) {
    ...
    try {
      ...
      if (err.inner) {
        ...
        setSnackbar({
          open: true,
          message:
            err.message || 'Por favor, corrija os erros antes de continuar',
          severity: 'error',
        });
      }
    }
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Fade in={true}>
        ...
        >
          ...
          <Typography variant="body2">
            {activeStep === 0 &&
              'Preencha seus dados com atenção. Um bom currículo começa com informações claras e profissionais.'}
            {activeStep === 1 &&
              'Liste suas experiências mais recentes primeiro. Descreva suas conquistas e responsabilidades de forma clara.'}
            {activeStep === 2 &&
              'Inclua sua formação acadêmica e cursos relevantes. Destaque certificações importantes para sua área.'}
            {activeStep === 3 &&
              'Liste habilidades técnicas e comportamentais relevantes. Seja honesto sobre seu nível de conhecimento.'}
          </Typography>
        </Alert>
      </Fade>
      ...
      {hasExistingData && activeStep === 0 && (
        <Fade in={true}>
            ...
            action={
              <Button
                ...
                onClick={() => {
                  if (
                    window.confirm(
                      'Tem certeza que deseja limpar todos os dados e começar um novo currículo?'
                    )
                  ) {
          ...
          >
            <Typography variant="body2">
              Você tem dados salvos de um currículo anterior. Deseja continuar
              editando ou começar um novo?
            </Typography>
          </Alert>
        </Fade>
      )}
      ...
      <Paper elevation={2} sx={{ p: { xs: 2, md: 4 }, mb: 3, borderRadius: 2 }}>
        <form onSubmit={formik.handleSubmit}>{getStepContent(activeStep)}</form>
      </Paper>
